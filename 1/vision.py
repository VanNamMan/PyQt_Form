...
def get_matrixCode(img):
    ...
    pass
def ptInRect(p,r):
    if InList(p[0],[r[0],r[2]]) and InList(p[1],[r[1],r[3]]):
        return True
    else:
        return False
...
def get_mean(img):
    ...
    return m
def binary(img,k=50,mode=cv2.THRESH_BINARY,useBlur=False,useOtsu=False,useMean=False,s=2):
    if len(img.shape) == 3:
    ...
    if useBlur:
        ...
        blur = img.copy()
    if useMean:
        k = int(s*get_mean(blur)[0][0])
    if useOtsu:
        ret,res = cv2.threshold(blur,k,255,mode|cv2.THRESH_OTSU)
    else:
        ret,res = cv2.threshold(blur,k,255,mode)
    return res
def removeBlobs(binary,area=(0,100),width=(0,20),height=(0,20)):
    A = []
