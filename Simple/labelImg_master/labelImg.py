...
class Master(QMainWindow, WindowMixin):
    ...
    def __init__(self,defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        resetAll = action(getStr('resetAll'), self.resetAll, None, 'resetall', getStr('resetAllDetail'),enabled=False)
        ...
        self.drawSquaresOption.setEnabled(False)
        ...
        addActions(self.menus.file,
                   (open, opendir, changeSavedir, openAnnotation, self.menus.recentFiles, save, save_format, saveAs, close, resetAll))
    ...
    def resizeEvent(self, event):
        if self.canvas and not self.image.isNull()\
           and self.zoomMode != self.MANUAL_ZOOM:
            self.adjustScale()
        super(Master, self).resizeEvent(event)
...
def get_main_app(argv=[]):
    ...
    win = Master(argv[1] if len(argv) >= 2 else None,
